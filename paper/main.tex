...
This revision adds a hazard-misspecification stress test that breaks the simulator's favorable assumption that stale-map hazards are exposure-driven. When the true closure process is resampled to be age-driven, age decay reaches 0.444 success while CEMD falls to 0.400, so the contribution is explicitly limited to settings where exposure is predictive of map invalidation.
...
A v2 hazard-misspecification stress test narrows the claim: CEMD remains strongest when hazards are exposure-driven, but when the true process is made age-driven, age decay outperforms CEMD.
\end{abstract}
...
\item A v2 hazard-misspecification stress test showing the failure boundary: when true hazards are age-driven rather than exposure-driven, age decay reaches 0.444 success and CEMD reaches 0.400.
...
\paragraph{V2 hazard-misspecification stress.}
Table~\ref{tab:hazard-misspecification} attacks the central modeling assumption directly. We resample the true closure process so hazards range from exposure-driven to age-driven, while CEMD still plans with exposure estimates. CEMD is near the oracle when exposure is causal, but it is not a universally better decay rule. At exposure share 0.00, the age-decay baseline reaches 0.444 success versus 0.400 for CEMD. The paper's supported claim is therefore causal: exposure-based decay is useful when exposure predicts stale-map invalidation, not when age or another variable is the true hazard source.

\input{../results/hazard_misspecification_table}
