...
8. **Strongest evidence:** A runnable 650-episode stale-map graph-navigation simulator plus 800 stress episodes. Main success rates: no decay 0.417, age decay 0.432, uncertainty-only 0.425, CEMD 0.549, oracle probability 0.591. CEMD also reduces blocked-edge encounters and regret in this exposure-driven setting.

9. **Biggest weaknesses:** The simulator's hazard model is exposure-driven by construction; no real robot data validates that exposure clocks predict actual stale-map errors. The literature sweep is metadata-based. Exposure priors may be hard to estimate in deployment. The method does not solve SLAM, localization drift, learned exposure estimation, or multi-agent interaction.

10. **Paper-readiness judgment:** Workshop. The mechanism and runnable evidence are coherent enough for a focused workshop submission, but a main-track ICLR-style submission would need real-world validation or a stronger benchmark suite with externally sourced change processes.
